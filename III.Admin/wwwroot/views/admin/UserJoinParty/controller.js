...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataserviceJoinParty, $location, $translate) {
    ...
    $scope.GetMemberPartyProfile = function (resumeNumber) {
        dataserviceJoinParty.GetMemberPartyProfile(resumeNumber, function (rs) {
            rs = rs.data;
            if (!rs.Error)
                $scope.downloadFileObj("Phiếu đảng viên", rs.Object);
            console.log(rs);
        });
    }


    $scope.getBriefCv = function (resumeNumber) {
        dataserviceJoinParty.GetMemberPartyProfile2(resumeNumber, function (rs) {
            rs = rs.data;
            if (!rs.Error)
                $scope.downloadFileObj("Lý lịch trích lược", rs.Object);
            console.log(rs);
        });
    }
    $scope.downloadFileObj = function (typePrefix = "Lý lịch trích lược", obj, extension = "docx") {
        console.log(obj);
        var link = document.createElement("a");
        link.href = obj.Path;
        link.download = `${typePrefix} ${obj.Profile.CurrentName} ${obj.Profile.Username}.${extension}`;
        link.click();
    }
    ...
    $scope.ImportFile = function (data) {
        dataserviceJoinParty.Import(data, function (rs) {
            ...
            if (rs.Error) {
                ...
                $scope.downloadFileObj("Sơ yếu lý lịch đầy đủ", rs.Object);
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('LastTimeReport').withOption('sClass', 'listaction text-center w50').withTitle('{{"Lý lịch trích lược" | translate}}')
        .renderWith(function (data, type, full) {
         ...
         `*/
            wfbtn = `
        <input type="checkbox" style=" width: 20px; height: 20px;" ng-model="Export['${full.resumeNumber}']"/>
        <a title="{{&quot;Tải Lý lịch trích lược&quot; | translate}}" class="width: 25px; height: 25px; padding: 0px"
            ng-click="getBriefCv('${full.resumeNumber}')"><i class="fa fa-file-word-o  fs25"></i>
        </a> `
...
app.controller('edit-user-join-party', function ($scope, $rootScope, $compile, $routeParams, dataserviceJoinParty, $filter, $uibModal, $http) {
    ...
    $scope.openPopover3 = function (popoverId) {
        ...
        if (pp) {
            ...
            $scope.pp = {
                id: `family_${$scope.selectedFamily.Id}`,
                comment: '',
                idFamily: {
                }
            };
    ...
    $scope.openPopover4 = function (popoverId) {
        ...
        if (pp) {
            ...
            $scope.pp = {
                id: `PersonHistory_${$scope.selectedPersonHistory.Id}`,
                comment: '',
                idFamily: {
                }
            };
    ...
    $scope.openPopover7 = function (popoverId) {
        ...
        const pp = $scope.jsonGuide.find(x => x.id === `laudatory_${$scope.selectedLaudatory.Id}`);
        matchedLabel = $scope.popoverLabels.find(function (item) {
            return item.id === popoverId;
        });
        if (pp) {
            ...
            $scope.pp = {
                id: `laudatory_${$scope.selectedLaudatory.Id}`,
                comment: '',
                idFamily: {
                }
            };
    ...
    $scope.openPopover11 = function (popoverId) {
        if (!$scope.selectedTrainingCertificatedPass || !$scope.selectedTrainingCertificatedPass.Id) {
            return;
        }
        const pp = $scope.jsonGuide.find(x => x.id === `TrainingCertificatedPass_${$scope.selectedTrainingCertificatedPass.Id}`);
        matchedLabel = $scope.popoverLabels.find(function (item) {
            return item.id === popoverId;
        });
        if (pp) {
            ...
            $scope.pp = {
                id: `TrainingCertificatedPass_${$scope.selectedTrainingCertificatedPass.Id}`,
                comment: '',
                idFamily: {
                }
            };
    ...
    $scope.ImportJsonFile = function (data) {
        ...
        $scope.thon_PlaceofBirth = data.Profile.BirthPlaceVillage;
        $scope.thon_HomeTown = data.Profile.HomeTownVillage;
        $scope.thon_Residence = data.Profile.PermanentResidenceVillage;
        $scope.thon_TemporaryAddress = data.Profile.thon_TemporaryAddress;
        $scope.infUser.TemporaryAddress = data.Profile.TemporaryAddressVillage;
        ...
        if (data.Profile.CreatedPlace) {
            var CreatedPlace = data.Profile.CreatedPlace.split("_")
            if (CreatedPlace.length == 2) {
                $scope.placeAddress = CreatedPlace[1];
                $scope.PlaceCreatedTime.place = CreatedPlace[0];
            }
        }
...
return;
    ...
    var updatePopoverLabels = function () {
        for (var i = 0; i < $scope.Relationship.length; i++) {
            var x = $scope.Relationship[i];
            $scope.popoverLabels.push({
                "id": "popup_" + x.id,
                "labelText": x.Relation + " - " + x.Name // Nội dung label
            });
        }
    ...
    $scope.selectFamily = function (x) {
        ...
        if ($scope.selectedFamily.HomeTown) {
            $scope.selectedFamily.HomeTown = $scope.selectedFamily.HomeTown
        } else {
            $scope.resetFamilyHomeTown();
        ...
        /*setTimeout(() => $scope.forms.addFamily.validate(), 150);*/
    ...
    $scope.resetFormFamily = function () {
                    ...
                    if (pE8[y][i].startsWith("- Quê quán:")) {
                        ...
                        if ($scope.Relationship[RelationshipIndex].HomeTown = pE8[y][i].slice(('- Quê quán:').length).trim()) {
                            ...
                            if (PlaceofBirth5.length == 4) {
                                ...
                                Promise.all([
                                    new Promise((resolve, reject) => {
                                        dataserviceJoinParty.GetTinhName(tempPlaceofBirth5[3], function (rs) {
                                            if (rs.data && rs.data[0]) {
                                    ...
                                    new Promise((resolve, reject) => {
                                        dataserviceJoinParty.getDistrictsName(tempPlaceofBirth5[2], tempPlaceofBirth5[3], function (rs) {
                                            if (rs.data && rs.data[0]) {
                                    ...
                                    new Promise((resolve, reject) => {
                                        dataserviceJoinParty.getWardsName(tempPlaceofBirth5[1], tempPlaceofBirth5[2], tempPlaceofBirth5[3], function (rs) {
                                            if (rs.data && rs.data[0]) {
                ...
                if ($scope.pageInfo[i].innerText.trim().startsWith('Nơi sinh:')) {
                    ...
                    if (PlaceofBirth.length == 4) {
                        Promise.all([
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getDistrictsName(PlaceofBirth[2], PlaceofBirth[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getWardsName(PlaceofBirth[1], PlaceofBirth[2], PlaceofBirth[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                ...
                if ($scope.pageInfo[i].innerText.trim().startsWith('Quê quán:')) {
                    ...
                    if (HomeTown.length == 4) {
                        Promise.all([
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getDistrictsName(HomeTown[2], HomeTown[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getWardsName(HomeTown[1], HomeTown[2], HomeTown[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                ...
                if ($scope.pageInfo[i].innerText.trim().startsWith('- Nơi thường trú :')) {
                    ...
                    if (Residence.length == 4) {
                        Promise.all([
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getDistrictsName(Residence[2], Residence[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getWardsName(Residence[1], Residence[2], Residence[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                ...
                if ($scope.pageInfo[i].innerText.trim().startsWith('- Nơi tạm trú :')) {
                    ...
                    if (TemporaryAddress.length == 4) {
                        Promise.all([
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getDistrictsName(TemporaryAddress[2], TemporaryAddress[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
                            ...
                            new Promise((resolve, reject) => {
                                dataserviceJoinParty.getWardsName(TemporaryAddress[1], TemporaryAddress[2], TemporaryAddress[3], function (rs) {
                                    if (rs.data && rs.data[0]) {
...
app.controller('comment', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, dataserviceJoinParty, para) {
    ...
    $scope.init = function () {
        ...
        $scope.pp.id = matchedLabel?.id;
        $scope.pp.comment = matchedLabel?.comment ?? '';
        $scope.pp.idFamily = matchedLabel?.idFamily ?? '';
        $scope.popoverid = matchedLabel?.id;
        if (matchedLabel) {
            $scope.popoverLabel = matchedLabel.labelText;
            $scope.commentTextarea = matchedLabel.comment;
